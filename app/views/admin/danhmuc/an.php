<div class="card">
    <div class="card-body">
        <!-- Hiện thị thông báo  -->
        <?php if (isset($erros['maSanPham'])) { ?>
            <div class='alert alert-danger' role='alert'> <?= $erros['maSanPham'] ?? '' ?> </div>
        <?php  } else ?>
        <?php if (isset($erros['maLoais'])) { ?>
            <div class='alert alert-danger' role='alert'> <?= $erros['maLoais'] ?? '' ?> </div>
        <?php  } ?>
        <!--  -->
        <form action="?url=admin/danhmuc/an&trang_thai=0&ten_danh_muc=<?=$tenDanhMucOld?>" method="post">
            <h5 class="card-title">Ẩn danh mục</h5>
